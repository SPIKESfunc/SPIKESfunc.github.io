...
function graphAlert(div, property) {
    ...
    if (property === "inv") {
        document.getElementById(div).innerHTML = "ERROR - Value too low to sustain curve.";
    }
}
...
var answersAgonist = ["Agonism is the process via which a ligand called the agonist binds to a receptor and alters the receptor state resulting in a biological response.<img src=\"images/agonistchart.png\" alt=\"Ago1 Answer\" style=\"max-height:100%; max-width:100%;\"> For more information, watch the ‘What are agonists?’ animation in the Videos tab.",
    ...
    "Potency is one expression of the activity of a drug, in terms of the concentration or amount needed to produce a defined effect.  Potency depends on both agonist-dependent (affinity, efficacy) and cell-dependent (R<sub>T</sub>, <i>&#947</i> ) parameters, and thus can vary significantly between different cells and tissues. The term <b>DOES NOT</b> refer to the maximum effect attainable.  The EC<sub>50</sub> value is a commonly used measure of agonist potency and is the molar concentration of the agonist that produces 50% of the maximum response induced by that agonist (NOT the [agonist] producing 50% max. effect). It is represented by grey circle on the agonist dose-response curve.  The pD<sub>2</sub> is another measure of agonist potency, and is simply the –logEC<sub>50</sub>.",
