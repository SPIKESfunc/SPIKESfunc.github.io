...
var questionsEff = [
  ...
  "Is the effect of an Allosteric Antagonist (affecting agonist efficacy) influenced by the properties of the agonist (Affinity, Intrinsic Efficacy) or the cell (R<sub>T</sub>, <i>&#947</i> )?<br><i>This can be tested using the Visualiser</i>",
...
var answersEff = [
  ...
  "<b>YES</b>, but this will dependent upon whether the agonist is a partial or full agonist.  For partial agonists, the allosteric antagonist will reduce the maximum response.  However, for a full agonist (acting on a cell with high R<sub>T</sub> and signal amplification), the allosteric antagonist (affecting agonist efficacy) will produce a rightward shift of the agonist dose-response curve (at low [allosteric antagonist]) prior to suppression of the maximum response (at higher [allosteric antagonist]). This effect can be observed using the visualiser.",
  "<b>YES</b>, the effects of an allosteric antagonist (affecting agonist efficacy) will be GREATER against agonists will low efficacy and in cells with low receptor densities (R<sub>T</sub>)and coupling efficiencies (<i>&#947</i>).  This effect can be observed using the visualiser.",
